...
def get_df_runs_from_mlflow_sql(engine, runs_columns, experiments_columns=None, experiments_names=None,
                                other_table=None, other_table_keys=None, pivot_other_table=True, lifecycle_stage='active'):
    ...
    # WHERE experiments.name IN ('experiment_name1', 'experiment_name2') AND other_table.key IN ('key1', 'key2') and runs.lifecycle_stage = 'active'
    ...
    if experiments_names or other_table_keys or lifecycle_stage:
        query += " WHERE"
        ...
        if lifecycle_stage:
            if experiments_names or other_table_keys:
                query += " AND"
            query += f" runs.lifecycle_stage = '{lifecycle_stage}'"
